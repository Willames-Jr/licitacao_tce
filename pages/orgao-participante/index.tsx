...
import { OrgaoParticipante } from "utils/types";
...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  const orgaos = await prisma.orgaoParticipante.findMany();    
  return {
...
const Licitacao: React.FC<Props> = ({orgaos}) => {
  ...
  const handleEdit = (event,rowData) => {
    Router.push({pathname: '/orgao-participante/create', query: {id: rowData.id}})
  }
  ...
  const handleDelete = async () => {
    ...
    let response;
    try{
        response = await fetch('/api/orgao-participante/'+encodeURIComponent(selectedItem ?? ""), {
            method: 'DELETE',
    ...
    NProgress.done();
    if (response?.ok) {
        Router.push('/orgao-participante/').then(() => {
          Router.reload();
        });
    }
  }
  ...
  return (
    <Layout>
      <PageTitle>Orgãos participantes</PageTitle>
      <div className="flex flex-col flex-wrap mb-8 space-y-4 md:flex-row md:items-end md:space-x-4">
        <Button iconLeft={Add} onClick={() => Router.push("/orgao-participante/create")} size="regular">Adicionar</Button>
      </div>

      <div className="mb-8">
                    
      <MaterialTable
        title="Orgãos participantes"
        ...
        actions={[
          {
            icon: EditOutlined,
            tooltip: 'Editar orgão',
            onClick:  handleEdit
          },
          {
            icon: DeleteOutlined,
            tooltip: 'Deletar orgão',
