...
import { ItemLicitacao, ItemLicitacaoValidation, Message } from "utils/types";
...
export async function getServerSideProps({ query }) {
    ...
    if (numero_item) {
      initialData = await prisma.itemLicitacao.findUnique({
        where: { numero_item },
      });
    }
...
const Create: React.FC<Props> = ({initialData,contratacoes, licitacoes}) => {
  
  const [isEditMode, setIsEditMode] = useState(!!initialData)
  const [validation, setValidation] = useState<ItemLicitacaoValidation>({
  ...
  const validFields = () => {
    let errors: ItemLicitacaoValidation = {
      numero_contratacaoId:    [],
      numero_licitacaoId:      [],
      numero_item:             [],
      descricao:               [],
    ...
    if ((numero_contratacaoId === "" && numero_licitacaoId === "") || (!numero_contratacaoId && !numero_licitacaoId) || (numero_contratacaoId === 0 && numero_licitacaoId === 0)) {
        errors.numero_contratacaoId.push("O campo Numero Contratação e/ou Número licitação devem ser preenchidos")
        errors.numero_licitacaoId.push("O campo Numero Contratação e/ou Número licitação devem ser preenchidos")
        isValid = false  
    }
    if (descricao === "") {
    ...
    if (unidade_medida === "") {
        errors.unidade_medida.push("O campo deve ser preenchido")
        isValid = false  
    }
    if (quantidade === "" || quantidade === "0.00") {
        errors.quantidade.push("O campo deve ser preenchido")
        isValid = false  
    }

    if(numero_licitacaoId !== "") {
      const licitacao = licitacoes.find(l => l.numero_licitacao === numero_licitacaoId)
      if (licitacao?.registro_preco && (valor_unitario_estimado === "" || valor_unitario_estimado === "0.00")){
        errors.valor_unitario_estimado.push("A licitação escolhida possui registro de preços, portanto esse campo deve ser preenchido") 
        isValid = false
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    if (isEditMode) {
      try{
          response = await fetch('/api/item-licitacao/'+encodeURIComponent(initialData.numero_item), {
              method: 'PUT',
      ...
      try{
          response = await fetch('/api/item-licitacao', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(body),
          });
    ...
    if (response?.ok) {
        setMessage({type:"success", text: `Item ${isEditMode ? 'atualizado' : 'cadastrado'} com sucesso !`})
        // setIsEditMode(true);
        // setPessoaAtual(body);
    } else {
    ...
    NProgress.done()
    if (isEditMode) {
      Router.push('/item-licitacao/create').then(() => Router.reload());
    }
    setBlocked(false)
  ...
  return (
    <Layout>
      ...
      <Text className="mb-4">* Cadastro de licitação ou contratação direta</Text>
      {
        message.text !== "" &&
