...
const Licitacao: React.FC<Props> = ({grupos}) => {
  ...
  const tableColumns = [
      ...
      {
          field: "codigo_participante.nome",
          title: "Proponente",
      }
  ]

  const handleEdit = (event,rowData) => {
    Router.push({pathname: '/propostas/create', query: {id: rowData.id}})
  }
  ...
  const handleDelete = async () => {
    ...
    let response;
    try{
        response = await fetch('/api/propostas/'+encodeURIComponent(selectedItem ?? ""), {
            method: 'DELETE',
  ...
  return (
    <Layout>
      ...
      <div className="flex flex-col flex-wrap mb-8 space-y-4 md:flex-row md:items-end md:space-x-4">
        <Button iconLeft={Add} onClick={() => Router.push("/propostas/create")} size="regular">Adicionar</Button>
      </div>
