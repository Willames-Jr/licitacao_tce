...
import { GrupoItemLicitacao } from "utils/types";
...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  const grupos = await prisma.grupoLicitacaoItem.findMany();    
  return {
...
const Licitacao: React.FC<Props> = ({grupos}) => {
  ...
  const handleEdit = (event,rowData) => {
    Router.push({pathname: '/grupo-item-licitacao/create', query: {numero_item: rowData.numero_item}})
  }
  ...
  const handleDelete = async () => {
    ...
    let response;
    try{
        response = await fetch('/api/grupo-item-licitacao/'+encodeURIComponent(selectedItem ?? ""), {
            method: 'DELETE',
    ...
    NProgress.done();
    if (response?.ok) {
        Router.push('/grupo-item-licitacao/').then(() => {
          Router.reload();
        });
    }
  }
  ...
  return (
    <Layout>
      <PageTitle>Items dos Grupos/Lotes de licitação</PageTitle>
      <div className="flex flex-col flex-wrap mb-8 space-y-4 md:flex-row md:items-end md:space-x-4">
        <Button iconLeft={Add} onClick={() => Router.push("/grupo-item-licitacao/create")} size="regular">Adicionar</Button>
      </div>

      <div className="mb-8">
                    
      <MaterialTable
        title="Items dos grupos/lotes de licitação"
